...
void main(uint3 dtid : SV_DispatchThreadID){
    ...
    float3 p = worldFromDepth(uv,depth);

    float4 gb0 = GBuffer0.Load(int3(px,0));
    float4 gb1 = GBuffer1.Load(int3(px,0));
    float3 emissive = GBuffer2.Load(int3(px,0)).rgb;

    float3 albedo = gb0.rgb;
    float metallic = gb0.a;
    float3 N = octDecode(gb1.rg);
    float roughness = max(gb1.b,0.04);
    ...
    for(uint i=0;i<lightCount;i++){
        ...
        LightSample s = evaluateLight(light,p);

        if(all(s.Li==0.0)) continue;

        float NdotL = dot(N,s.Lo);
        if(NdotL <= 0.0) continue;

        float3 brdf = evaluateBRDF(N,V,s.Lo,albedo,metallic,roughness);
